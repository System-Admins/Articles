...
You could do a backup and restore, but that is time-consuming and tedious work. Microsoft has pointed out [in this article](https://docs.microsoft.com/en-us/azure/azure-sql/database/database-copy?view=azuresql&tabs=azure-powershell#copy-to-a-different-subscription) it can "only" be done by using T-SQL. Instead of manually going through this process every time, I decided to create a PowerShell script that encapsulates the SQL commands necessary to do the copy across subscriptions.
...
In the following code which also [can be found on GitHub](https://github.com/System-Admins/PowerShell/blob/main/scripts/microsoft/azure/database/azure-sql-database/Copy-AzureSQLDatabase.ps1), you only need to adjust from line 31 to 61 according to your environment.
